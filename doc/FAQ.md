...
Some tables uses trigger to populate some non-null columns, e.g. primary key value is based on a sequence and populated by a insert trigger.  For this situation, you can turn this parameter to "Y" in order to by-pass this checking.
...
Create a template from a database view. Remove RESULT column.  Set `CREATE_ROWID_COLUMN`, `CONFIRM_OPERATION`, `KEEP_PARAMETER_WORKSHEET` to "N". Set the `NEW_FILE_NAME` to a name containing a timestamp, e.g. $D{YYYY-MM-DD}.  Run this template using command line (EXZELLENZ.bat).  Then distribute the result file by other scripts.

### How can I turn on the debug mode to see more details if I encounter an error ?

In GUI mode, from the pop-up menu you can change the Log Level to Debug.  Or you can change the parameter LOG_LEVEL in the configuration file EXZ.properties.  The settings will be used in the next run.

### What is the password to unlock the EXZELLENZ worksheet ?
The password is `symbolthree.exzellenz`.

### How the application knows the database user password is encrypted ?
The encrypted password is always 50 character long, starting with `ZG`. It will be encrypted by the program if it is not the case.
