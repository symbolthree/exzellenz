...
- In the template worksheet, you can delete any columns (except `RESULT` column, which is available for table, but not for view) you don't need it.
...
- For UPDATE and DELETE operations, you have to copy the cell belonging to `PENDING_UPDATE` and `PENDING_DELETE` to the RESULT column in the template worksheet. Only these "flagged" Excel data rows will be picked up for processing.
