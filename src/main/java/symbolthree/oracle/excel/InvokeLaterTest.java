...
public class InvokeLaterTest extends JPanel {

  private static final long serialVersionUID = -7365577817033360447L;
    ...
    private class LongRunningModelFillAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            InvokeLaterTest.this.listModel.removeAllElements();
            InvokeLaterTest.this.listModel.addElement("Calculating...");
    ...
    private class PopulationRunnable implements Runnable {
        public void run() {
            ...
            for (int i = 1; i <= 100; i++) {
                ...
                if ((i % 10) == 0) {
                    ...
                    SwingUtilities.invokeLater(new Runnable() {
                        public void run() {
                            InvokeLaterTest.this.statusArea.setText("Calculated " + progress);
                        }
                    });
                }
                try {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    InvokeLaterTest.this.statusArea.setText("");
                    InvokeLaterTest.this.listModel.removeAllElements();
                    for (int i = 0; i < values.length; i++) {
                        InvokeLaterTest.this.listModel.addElement(values[i]);
                    }
                }
            });
        }
